...
@views function porous_convection_2D()
    ...
    β_dτ_D  = kvf * re_D / (cfl * min(dx , dy) * max(lx, ly))
    ...
    for it in 1:nt
        ...
        while err_Pf >= ϵtol && iter <= maxiter
            # pressure
            qDx[2:end-1, :] .-= (qDx[2:end-1, :] .+ kvf .* (diff(P, dims=1) ./ dx .- α .* avx(T))) ./ (θ_dτ_D + 1.0)
            qDy[:, 2:end-1] .-= (qDy[:, 2:end-1] .+ kvf .* (diff(P, dims=2) ./ dy .- α .* avy(T))) ./ (θ_dτ_D + 1.0)
        ...
        dta = min(dy,dx) / max(maximum(abs.(qDx)), maximum(abs.(qDy))) / 1.1
        dtd = min(dy,dx)^2 / λ / 2.1
        dt  = min(dta, dtd)
        # temperature

        #temperature diffusion
        T[2:end-1, :] .+= dt .* diff(λ .* diff(T, dims=1) ./ dx, dims=1) ./ dx
        T[:, 2:end-1] .+= dt .* diff(λ .* diff(T, dims=2) ./ dy, dims=2) ./ dy
