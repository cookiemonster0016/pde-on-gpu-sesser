...
function update_Pf_t!(nx, ny, Pf, qDx, qDy, _β_dτ_dx, _β_dτ_dy)
    ...
    #= GPU Kernels =#
@parallel_indices (ix, iy) function update_Pf!(nx, ny, Pf, qDx, qDy, _β_dτ_dx, _β_dτ_dy)
...
@parallel_indices (ix, iy) function compute_flux!(nx, ny, qDx, qDy, Pf, k_ηf_dx, k_ηf_dy, _1_θ_dτ)
    nx, ny = size(Pf)
   ...
   # physics
    ...
    while err_Pf >= ϵtol && iter <= maxiter && do_check
        ...
        if useThreads
            ...
            @parallel compute!(nx, ny, qDx, qDy, Pf, k_ηf_dx, k_ηf_dy, _1_θ_dτ, _β_dτ_dx, _β_dτ_dy, threads, blocks)
        end
...
#todo add do_check and plotting/ error checking
