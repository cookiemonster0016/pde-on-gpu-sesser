...
@views function diffusion_1D_2procs(; do_visu=false)
    ...
    for it = 1:nt
        ...
        if do_visu
            ...
            p = plot(Cg, legend=false, linewidth=0, markershape=:circle, markersize=5, yaxis=font(fontsize, "Courier"), xaxis=font(fontsize, "Courier"), titlefontsize=fontsize, titlefont="Courier")
            display(plot!(C, legend=false, linewidth=3, framestyle=:box, xlabel="Lx", ylabel="C", title="diffusion (it=$(it))"))
            if it == nt
                savefig(p, "diffusion_1D_2procs.png")
            end
        end
    end
    return
