...
@views function diffusion_1D_nprocs(; do_visu=true)
    ...
    for ip = 1:np
        for ix = 1:nx
            x[ix, ip] = dxg * ((ip-1)*(nx-2) + ix) -0.5 * dxg - 0.5 * lx
    ...
    for it = 1:nt
        ...
        if do_visu
            fontsize = 12
            p= plot(xt, Ct, legend=false, linewidth=1, markershape=:circle, markersize=3, yaxis=font(fontsize, "Courier"), xaxis=font(fontsize, "Courier"), titlefontsize=fontsize, titlefont="Courier")
            ...
            for ip = 1:np
                ...
                if it == nt
                    savefig(p, "diffusion_1D_nprocs.png")
                end
            end
        end
    end
