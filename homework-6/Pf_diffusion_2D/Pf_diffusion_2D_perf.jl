...
module perf
    ...
    function compute!(qDx, qDy, Pf, k_ηf_dx, k_ηf_dy, _1_θ_dτ,_β_dτ, _dx, _dy)
    ...
    function Pf_diffusion_2D(nx, ny, maxiter;do_check = false, usebm=true)
        # physics
        lx, ly = 20.0, 20.0
        k_ηf   = 1.0
        # numerics
        ...
        else#use the benchmark tool for timing
            t_it = @belapsed compute!($qDx, $qDy, $Pf, $k_ηf_dx, $k_ηf_dy, $_1_θ_dτ,$_β_dτ, $_dx, $_dy)
            Aeff = 2*3 * nx*ny * 8 / 1e9 #2 = read and write, 3 = amount of arrays, nx"ny gridsize, *8/1e9 convert to Gb
            Teff = Aeff / t_it
            return Teff
        end
