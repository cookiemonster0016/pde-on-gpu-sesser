...
module loop
    ...
    function compute!(nx, ny, qDx, qDy, Pf, k_ηf_dx, k_ηf_dy, _1_θ_dτ, _β_dτ_dx, _β_dτ_dy, useThreads)
        if useThreads 
    ...
    function Pf_diffusion_2D(nx, ny , maxiter;do_check = false, use_threads=false)
        # physics
        lx, ly = 20.0, 20.0
        k_ηf   = 1.0
        # numerics
        ...
        while err_Pf >= ϵtol && iter <= maxiter && do_check
            ...
            compute!(nx, ny, qDx, qDy, Pf, k_ηf_dx, k_ηf_dy, _1_θ_dτ, _β_dτ_dx, _β_dτ_dy, use_threads)
        ...
        if !do_check
            ...
            t_it = @belapsed compute!($nx, $ny, $qDx, $qDy, $Pf, $k_ηf_dx, $k_ηf_dy, $_1_θ_dτ, $_β_dτ_dx, $_β_dτ_dy, $use_threads) 
        else
